...
class CorrectorEmpresaula:
    ...
    def corregir_compras(self, fecha_entrega_str=None):
        ...
        if fecha_entrega_str:
            fecha_entrega = normalizar_fecha(fecha_entrega_str)
            info(f"Fecha de entrega del trabajo: {c(str(fecha_entrega), Color.YELLOW)}")
        ...
        df_real["R_FECHA_EMISION_C"] = df_real["R_FECHA_EMISION_C"].apply(
            normalizar_fecha
        )
        ...
        df_ped["A_FECHA_EMISION_CP"] = df_ped["A_FECHA_EMISION_CP"].apply(
            normalizar_fecha
        )
        df_ped["A_FECHA_ALTA_ODOO_CP"] = df_ped["A_FECHA_ALTA_ODOO_CP"].apply(
            normalizar_fecha
        )
        ...
        df_alb["A_FECHA_EMISION_CA"] = df_alb["A_FECHA_EMISION_CA"].apply(
            normalizar_fecha
        )
        ...
        df_fac["A_FECHA_EMISION_CF"] = df_fac["A_FECHA_EMISION_CF"].apply(
            normalizar_fecha
        )
        ...
        for empresa in sorted(empresas):
            ...
            if not pedidos_faltantes:
                ...
                self._registrar(
                    empresa,
                    "PEDIDOS",
                    f"Todos los pedidos presentes ({len(nums_reales)})",
                    "ok",
                )
            else:
                for num in sorted(pedidos_faltantes):
                    ...
                    err(
                        f"PEDIDO FALTANTE: Nº {c(num, Color.YELLOW)} | {real['R_PROVEEDOR_C']} | {real['R_FECHA_EMISION_C']} | {real['R_IMPORTE_C']:.2f}€"
                    )
                    self._registrar(
                        empresa,
                        "PEDIDO_FALTANTE",
                        f"Pedido Nº{num} ({real['R_PROVEEDOR_C']}, {real['R_IMPORTE_C']:.2f}€)",
                    )
                    errores_emp += 1
            ...
            if pedidos_extra:
                for num in sorted(pedidos_extra):
                    ...
                    self._registrar(
                        empresa,
                        "PEDIDO_EXTRA",
                        f"Pedido Nº{num} no existe en EmpresAula",
                        "aviso",
                    )
            ...
            for _, real_row in reales_emp.iterrows():
                ...
                if ped["A_PROVEEDOR_CP"].upper() != real_row["R_PROVEEDOR_C"].upper():
                    err(
                        f"Pedido {num_cp}: PROVEEDOR incorrecto → alumno: '{ped['A_PROVEEDOR_CP']}' | real: '{real_row['R_PROVEEDOR_C']}'"
                    )
                    self._registrar(
                        empresa,
                        "PROVEEDOR_ERROR",
                        f"Pedido {num_cp}: alumno '{ped['A_PROVEEDOR_CP']}' vs real '{real_row['R_PROVEEDOR_C']}'",
                    )
                    errores_emp += 1
                ...
                if ped["A_FECHA_EMISION_CP"] != real_row["R_FECHA_EMISION_C"]:
                    err(
                        f"Pedido {num_cp}: FECHA EMISIÓN incorrecta → alumno: {ped['A_FECHA_EMISION_CP']} | real: {real_row['R_FECHA_EMISION_C']}"
                    )
                    self._registrar(
                        empresa,
                        "FECHA_ERROR",
                        f"Pedido {num_cp}: {ped['A_FECHA_EMISION_CP']} vs {real_row['R_FECHA_EMISION_C']}",
                    )
                    errores_emp += 1
                ...
                if abs(imp_real - imp_alu) > 0.02:
                    err(
                        f"Pedido {num_cp}: IMPORTE incorrecto → alumno: {imp_alu:.2f}€ | real: {imp_real:.2f}€"
                    )
                    self._registrar(
                        empresa,
                        "IMPORTE_ERROR",
                        f"Pedido {num_cp}: {imp_alu:.2f}€ vs {imp_real:.2f}€",
                    )
                    errores_emp += 1
                ...
                if estado_cp != "Pedido de compra":
                    err(
                        f"Pedido {num_cp}: ESTADO incorrecto → '{estado_cp}' (debe ser 'Pedido de compra')"
                    )
                    self._registrar(
                        empresa,
                        "ESTADO_ERROR",
                        f"Pedido {num_cp}: estado '{estado_cp}'",
                    )
                    errores_emp += 1
                ...
                if fecha_entrega and fecha_emision:
                    ...
                    if estado_fac != esperado:
                        warn(
                            f"Pedido {num_cp}: ESTADO FACTURACIÓN '{estado_fac}' → se esperaba '{esperado}' (emisión:{fecha_emision}, entrega:{fecha_entrega})"
                        )
                        self._registrar(
                            empresa,
                            "ESTADO_FAC",
                            f"Pedido {num_cp}: '{estado_fac}' vs esperado '{esperado}'",
                            "aviso",
                        )
            ...
            if not sin_albaran:
                ...
                self._registrar(
                    empresa, "ALBARANES", f"Trazabilidad pedido-albarán completa", "ok"
                )
            else:
                for ref in sorted(sin_albaran):
                    ped_row = peds_emp[peds_emp["A_REF_ODOO_CP"] == ref]
                    desc = (
                        f"Pedido {ped_row.iloc[0]['A_NUMERO_CP']}"
                        if not ped_row.empty
                        else ref
                    )
                    err(
                        f"ALBARÁN FALTANTE para {c(desc, Color.YELLOW)} (ref ODOO: {ref})"
                    )
                    self._registrar(
                        empresa,
                        "ALBARAN_FALTANTE",
                        f"Sin albarán para {desc} (ref:{ref})",
                    )
                    errores_emp += 1
            ...
            for _, alb in albs_emp.iterrows():
                ...
                if ped_orig.empty:
                    warn(
                        f"Albarán {alb['A_NUMERO_CA']}: origen '{ref_origen}' no corresponde a ningún pedido"
                    )
                    continue
                ...
                # Proveedor albarán
                if (
                    str(alb["A_PROVEEDOR_CA"]).strip().upper()
                    != real_row["R_PROVEEDOR_C"].upper()
                ):
                    err(
                        f"Albarán {alb['A_NUMERO_CA']}: PROVEEDOR incorrecto → '{alb['A_PROVEEDOR_CA']}' vs '{real_row['R_PROVEEDOR_C']}'"
                    )
                    self._registrar(
                        empresa,
                        "ALB_PROVEEDOR",
                        f"Albarán {alb['A_NUMERO_CA']}: proveedor incorrecto",
                    )
                    errores_emp += 1

                # Número albarán
                if (
                    str(alb["A_NUMERO_CA"]).strip()
                    != str(real_row["R_NUMERO_CA"]).strip()
                ):
                    err(
                        f"Albarán: NÚMERO incorrecto → alumno: {alb['A_NUMERO_CA']} | real: {real_row['R_NUMERO_CA']}"
                    )
                    self._registrar(
                        empresa,
                        "ALB_NUMERO",
                        f"Albarán Nº{alb['A_NUMERO_CA']} vs real Nº{real_row['R_NUMERO_CA']}",
                    )
                    errores_emp += 1
                ...
                if alb["A_FECHA_EMISION_CA"] != real_row["R_FECHA_EMISION_C"]:
                    err(
                        f"Albarán {alb['A_NUMERO_CA']}: FECHA incorrecta → alumno: {alb['A_FECHA_EMISION_CA']} | real: {real_row['R_FECHA_EMISION_C']}"
                    )
                    self._registrar(
                        empresa,
                        "ALB_FECHA",
                        f"Albarán {alb['A_NUMERO_CA']}: {alb['A_FECHA_EMISION_CA']} vs {real_row['R_FECHA_EMISION_C']}",
                    )
                    errores_emp += 1
                ...
                if abs(imp_real - imp_alb) > 0.02:
                    err(
                        f"Albarán {alb['A_NUMERO_CA']}: IMPORTE incorrecto → {imp_alb:.2f}€ vs {imp_real:.2f}€"
                    )
                    self._registrar(
                        empresa,
                        "ALB_IMPORTE",
                        f"Albarán {alb['A_NUMERO_CA']}: {imp_alb:.2f}€ vs {imp_real:.2f}€",
                    )
                    errores_emp += 1
                ...
                if alb["A_ESTADO_CA"] != "Hecho":
                    err(
                        f"Albarán {alb['A_NUMERO_CA']}: ESTADO '{alb['A_ESTADO_CA']}' → debe ser 'Hecho'"
                    )
                    self._registrar(
                        empresa,
                        "ALB_ESTADO",
                        f"Albarán {alb['A_NUMERO_CA']}: estado '{alb['A_ESTADO_CA']}'",
                    )
                    errores_emp += 1
            ...
            for _, ped in peds_emp.iterrows():
                ...
                if factura_requerida:
                    if ref_ped not in refs_fac_origen:
                        err(
                            f"FACTURA FALTANTE para pedido {ped['A_NUMERO_CP']} (ref: {ref_ped})"
                        )
                        self._registrar(
                            empresa,
                            "FACTURA_FALTANTE",
                            f"Sin factura para pedido {ped['A_NUMERO_CP']}",
                        )
                        errores_emp += 1
                    else:
                        ...
                        # Número factura
                        if (
                            str(fac_row["A_NUMERO_CF"]).strip()
                            != str(real_row["R_NUMERO_CF"]).strip()
                        ):
                            err(
                                f"Factura pedido {num_ped}: NÚMERO incorrecto → alumno: {fac_row['A_NUMERO_CF']} | real: {real_row['R_NUMERO_CF']}"
                            )
                            self._registrar(
                                empresa,
                                "FAC_NUMERO",
                                f"Pedido {num_ped}: factura {fac_row['A_NUMERO_CF']} vs real {real_row['R_NUMERO_CF']}",
                            )
                            errores_emp += 1
                        ...
                        if abs(imp_real - imp_fac) > 0.02:
                            err(
                                f"Factura {fac_row['A_NUMERO_CF']}: IMPORTE incorrecto → {imp_fac:.2f}€ vs {imp_real:.2f}€"
                            )
                            self._registrar(
                                empresa,
                                "FAC_IMPORTE",
                                f"Factura {fac_row['A_NUMERO_CF']}: {imp_fac:.2f}€ vs {imp_real:.2f}€",
                            )
                            errores_emp += 1

                        # Fecha factura (debe coincidir con fecha pedido)
                        if (
                            fac_row["A_FECHA_EMISION_CF"]
                            != real_row["R_FECHA_EMISION_C"]
                        ):
                            err(
                                f"Factura {fac_row['A_NUMERO_CF']}: FECHA incorrecta → {fac_row['A_FECHA_EMISION_CF']} vs {real_row['R_FECHA_EMISION_C']}"
                            )
                            self._registrar(
                                empresa,
                                "FAC_FECHA",
                                f"Factura {fac_row['A_NUMERO_CF']}: fecha incorrecta",
                            )
                            errores_emp += 1
                        ...
                        if estado_pago not in ("Publicado", "Pagada"):
                            err(
                                f"Factura {fac_row['A_NUMERO_CF']}: ESTADO PAGO '{estado_pago}' → debe ser 'Publicado' o 'Pagada'"
                            )
                            self._registrar(
                                empresa,
                                "FAC_ESTADO",
                                f"Factura {fac_row['A_NUMERO_CF']}: estado '{estado_pago}'",
                            )
                            errores_emp += 1
                else:
                    info(
                        f"Pedido {ped['A_NUMERO_CP']}: factura no requerida (emisión = fecha entrega)"
                    )
            ...
            if errores_emp == 0:
                ok(
                    c(
                        f"EMPRESA {empresa}: SIN ERRORES EN COMPRAS",
                ...
                err(
                    c(
                        f"EMPRESA {empresa}: {errores_emp} ERROR(ES) EN COMPRAS",
    ...
    def corregir_ventas(self, fecha_entrega_str=None):
        ...
        for nombre, df in [
            ("5_DATOS_VENTAS_REALES.csv", df_real),
        ...
        df_real["R_FECHA_EMISION_VP"] = df_real["R_FECHA_EMISION_VP"].apply(
            normalizar_fecha
        )
        df_real["R_FECHA_MAX_FACTURACION_V"] = df_real[
        ...
        df_ped["A_FECHA_EMISION_VP"] = df_ped["A_FECHA_EMISION_VP"].apply(
            normalizar_fecha
        )
        df_ped["A_FECHA_ALTA_ODOO_VP"] = df_ped["A_FECHA_ALTA_ODOO_VP"].apply(
            normalizar_fecha
        )
        ...
        df_alb["A_FECHA_ALBARAN_VA"] = df_alb["A_FECHA_ALBARAN_VA"].apply(
            normalizar_fecha
        )
        ...
        df_fac["A_FECHA_FACTURA_VF"] = (
            df_fac["A_FECHA_FACTURA_VF"].apply(normalizar_fecha)
            if "A_FECHA_FACTURA_VF" in df_fac.columns
            else df_fac.get("A_FECHA_ALBARAN_VA", pd.Series()).apply(normalizar_fecha)
        )
        ...
        df_fac["A_NUM_FACTURA_VF"] = (
            df_fac["A_NUM_FACTURA_VF."].astype(str).str.strip()
            if "A_NUM_FACTURA_VF." in df_fac.columns
            else df_fac.get("A_NUM_FACTURA_VF", pd.Series(dtype=str))
        )
        ...
        for empresa in sorted(empresas):
            ...
            if not faltantes:
                ...
                for num in sorted(faltantes):
                    ...
                    err(
                        f"PEDIDO FALTA: Nº {c(num, Color.YELLOW)} | {real['R_CLIENTE_V']} | {real['R_FECHA_EMISION_VP']} | {real['R_IMPORTE_V']:.2f}€"
                    )
            ...
            for _, real_row in reales_emp.iterrows():
                ...
                if ped["A_CLIENT_VP"].upper() != real_row["R_CLIENTE_V"].upper():
                    err(
                        f"Pedido venta {num_vp}: CLIENTE incorrecto → '{ped['A_CLIENT_VP']}' vs '{real_row['R_CLIENTE_V']}'"
                    )
                    self._registrar(
                        empresa, "PED_V_CLIENTE", f"Pedido {num_vp}: cliente incorrecto"
                    )
                    errores_emp += 1
                ...
                if ped["A_FECHA_EMISION_VP"] != real_row["R_FECHA_EMISION_VP"]:
                    err(
                        f"Pedido venta {num_vp}: FECHA → alumno: {ped['A_FECHA_EMISION_VP']} | real: {real_row['R_FECHA_EMISION_VP']}"
                    )
                    self._registrar(
                        empresa, "PED_V_FECHA", f"Pedido {num_vp}: fecha incorrecta"
                    )
                    errores_emp += 1
                ...
                if abs(ped["A_IMPORTE_VP"] - real_row["R_IMPORTE_V"]) > 0.02:
                    err(
                        f"Pedido venta {num_vp}: IMPORTE → {ped['A_IMPORTE_VP']:.2f}€ vs {real_row['R_IMPORTE_V']:.2f}€"
                    )
                    self._registrar(
                        empresa, "PED_V_IMPORTE", f"Pedido {num_vp}: importe incorrecto"
                    )
                    errores_emp += 1
                ...
                if estado != "Pedido de venta":
                    err(
                        f"Pedido venta {num_vp}: ESTADO '{estado}' → debe ser 'Pedido de venta'"
                    )
                    errores_emp += 1
                ...
                if fecha_alta_odoo and fecha_max and fecha_alta_odoo > fecha_max:
                    err(
                        f"Pedido venta {num_vp}: introducido en ODOO ({fecha_alta_odoo}) DESPUÉS del límite de facturación ({fecha_max})"
                    )
                    self._registrar(
                        empresa,
                        "PED_V_TARDIO",
                        f"Pedido {num_vp}: introducido tardíamente",
                    )
                    errores_emp += 1
            ...
            if not sin_albaran_v:
                ...
                self._registrar(
                    empresa, "ALB_VENTA", "Trazabilidad pedido-albarán venta OK", "ok"
                )
            else:
                for ref in sorted(sin_albaran_v):
                    ped_row = peds_emp[peds_emp["A_REF_ODOO_VP"] == ref]
                    desc = (
                        f"pedido {ped_row.iloc[0]['A_NUMERO_VP']}"
                        if not ped_row.empty
                        else ref
                    )
                    err(f"ALBARÁN VENTA FALTANTE para {desc}")
                    self._registrar(
                        empresa, "ALB_V_FALTANTE", f"Sin albarán venta para {desc}"
                    )
                    errores_emp += 1
            ...
            for _, alb in albs_emp.iterrows():
                ...
                if ped_orig.empty:
                    warn(
                        f"Albarán venta {alb.get('A_REF_ODOO_VA', '?')}: origen '{ref_origen}' no identificado"
                    )
                    continue
                ped_o = ped_orig.iloc[0]
                ...
                if f_alta and f_alb and f_alta > f_alb:
                    err(
                        f"Albarán venta pedido {num_vp}: fecha albarán ({f_alb}) ANTERIOR a alta ODOO del pedido ({f_alta})"
                    )
                    errores_emp += 1

                if f_alb and f_max and f_alb > f_max:
                    err(
                        f"Albarán venta pedido {num_vp}: generado ({f_alb}) DESPUÉS del límite viernes ({f_max})"
                    )
                    self._registrar(
                        empresa,
                        "ALB_V_TARDIO",
                        f"Albarán pedido {num_vp}: generado tardíamente",
                    )
                    errores_emp += 1
                ...
                if abs(alb["A_IMPORTE_VA"] - imp_real) > 0.02:
                    err(
                        f"Albarán venta pedido {num_vp}: IMPORTE → {alb['A_IMPORTE_VA']:.2f}€ vs {imp_real:.2f}€"
                    )
                    errores_emp += 1
                ...
                if alb["A_ESTADO_VA"] != "Hecho":
                    err(
                        f"Albarán venta pedido {num_vp}: ESTADO '{alb['A_ESTADO_VA']}' → debe ser 'Hecho'"
                    )
                    errores_emp += 1
                ...
                if alb["A_CLIENT_VA"].upper() != real_row["R_CLIENTE_V"].upper():
                    err(
                        f"Albarán venta pedido {num_vp}: CLIENTE '{alb['A_CLIENT_VA']}' vs '{real_row['R_CLIENTE_V']}'"
                    )
                    errores_emp += 1

            # ── 3. Facturas de venta ──
            print(f"  {c('3. FACTURAS DE VENTA', Color.CYAN)}")
            self._verificar_facturas_venta(
                empresa, peds_emp, facs_emp, reales_emp, errores_emp
            )

        subheader("Verificación de ventas completada")

    def _verificar_facturas_venta(
        self, empresa, peds_emp, facs_emp, reales_emp, errores_emp
    ):
        ...
        if not sin_factura:
            ...
            for ref in sorted(sin_factura):
                ...
                self._registrar(
                    empresa, "FAC_V_FALTANTE", f"Sin factura venta para {desc}"
                )
        ...
        if "A_NUM_FACTURA_VF" in facs_emp.columns or not facs_emp.empty:
            ...
            col_fecha = (
                "A_FECHA_FACTURA_VF"
                if "A_FECHA_FACTURA_VF" in facs_emp.columns
                else "A_FECHA_ALBARAN_VA"
            )
            ...
            if col_num in facs_emp.columns and col_fecha in facs_emp.columns:
                ...
                facs_sorted = facs_sorted.dropna(subset=["_orden"]).sort_values(
                    "_orden"
                )
                ...
                for i in range(len(ordenes)):
                    ...
                    if i > 0 and ordenes[i] != ordenes[i - 1] + 1:
                        err(
                            f"NUMERACIÓN NO CORRELATIVA: falta factura Nº{ordenes[i-1]+1} (salta de {ordenes[i-1]} a {ordenes[i]})"
                        )
                        self._registrar(
                            empresa,
                            "FAC_NUM_CORRELATIVA",
                            f"Falta factura Nº{ordenes[i-1]+1}",
                        )
                        errores_num += 1
                    ...
                    if (
                        ...
                        and fechas[i]
                        and fechas[i - 1]
                        and fechas[i] < fechas[i - 1]
                    ):
                        err(
                            f"FECHA NO CORRELATIVA: factura Nº{ordenes[i]} ({fechas[i]}) es anterior a Nº{ordenes[i-1]} ({fechas[i-1]})"
                        )
                        self._registrar(
                            empresa,
                            "FAC_FECHA_ORDEN",
                            f"Factura Nº{ordenes[i]}: fecha anterior a anterior",
                        )
                        errores_num += 1

                if errores_num == 0:
                    ok(
                        f"Numeración y fechas de facturas de venta: correctas ({len(ordenes)} facturas)"
                    )
                    self._registrar(
                        empresa, "FAC_V_NUM", "Numeración correlativa OK", "ok"
                    )

                # Estado pago facturas
                col_pagada = (
                    "A_PAGADA_VF" if "A_PAGADA_VF" in facs_emp.columns else None
                )
                if col_pagada:
                    for _, fac in facs_emp.iterrows():
                        ...
                        if estado not in ("Publicado", "Pagada", "Pagado parcialmente"):
                            err(
                                f"Factura {fac[col_num]}: ESTADO '{estado}' → debe ser 'Publicado' o 'Pagada'"
                            )
    ...
    def corregir_inventario(self, fecha_entrega_str=None):
        ...
        for nombre, df in [
            ("9_DATOS_INVENTARIO_ALUMNO.csv", df_inv),
        ...
        MAP_INV = {
            ...
            "UNITATS PENDENTS ENTRAR": [
                "A_UNITATS_PENDENTS_ENTRAR_I",
                "UNITATS PENDENTS ENTRAR",
                "UNIDADES PENDIENTES ENTRAR",
                "Incoming",
                "incoming",
                "INCOMING",
            ],
            "UNITATS PENDENTS SORTIR": [
                "A_UNITATS_PENDENTS_SORTIR_I",
                "UNITATS PENDENTS SORTIR",
                "UNIDADES PENDIENTES SALIR",
                "Outgoing",
                "outgoing",
                "OUTGOING",
            ],
        ...
        df_inv["UNITATS REALS"] = (
            pd.to_numeric(df_inv["UNITATS REALS"], errors="coerce")
            .fillna(0)
            .astype(int)
        )
        df_inv["UNITATS DISPONIBLES"] = (
            pd.to_numeric(
                df_inv.get("UNITATS DISPONIBLES", pd.Series(0, index=df_inv.index)),
                errors="coerce",
            )
            .fillna(0)
            .astype(int)
        )
        df_inv["UNITATS PENDENTS ENTRAR"] = (
            pd.to_numeric(
                df_inv.get("UNITATS PENDENTS ENTRAR", pd.Series(0, index=df_inv.index)),
                errors="coerce",
            )
            .fillna(0)
            .astype(int)
        )
        df_inv["UNITATS PENDENTS SORTIR"] = (
            pd.to_numeric(
                df_inv.get("UNITATS PENDENTS SORTIR", pd.Series(0, index=df_inv.index)),
                errors="coerce",
            )
            .fillna(0)
            .astype(int)
        ...
        df_his["Cantidad"] = (
            pd.to_numeric(df_his["Cantidad"], errors="coerce").fillna(0).astype(int)
        )
        ...
        for empresa in sorted(empresas):
            ...
            if total_inventario <= 1000.0:
                ok(
                    f"Valor total inventario: {c(f'{total_inventario:.2f}€', Color.GREEN)} (límite: 1.000€)"
                )
                self._registrar(
                    empresa,
                    "INV_VALOR",
                    f"Inventario {total_inventario:.2f}€ ≤ 1.000€",
                    "ok",
                )
            else:
                err(
                    f"Valor total inventario: {c(f'{total_inventario:.2f}€', Color.RED)} SUPERA el límite de 1.000€"
                )
                self._registrar(
                    empresa,
                    "INV_VALOR",
                    f"Inventario {total_inventario:.2f}€ supera 1.000€",
                )
                errores_emp += 1
            ...
            for _, row in inv_emp.iterrows():
                ...
                if abs(imp - calc) > 0.02 and not (imp == 0 and unid == 0):
                    err(
                        f"Producto {row['PRODUCTE']}: IMPORT {imp:.2f}€ ≠ {coste:.2f}€ × {unid} = {calc:.2f}€"
                    )
                    errores_emp += 1
            ...
            if fecha_entrega and fecha_entrega.weekday() == 4:  # 4 = viernes
                for _, row in inv_emp.iterrows():
                    ...
                    if pend_ent != 0 or pend_sal != 0:
                        err(
                            f"Producto {row['PRODUCTE']}: PENDIENTES en viernes → Entrar:{pend_ent} / Salir:{pend_sal}"
                        )
                        self._registrar(
                            empresa,
                            "INV_PENDIENTES",
                            f"{row['PRODUCTE']}: pendientes en viernes",
                        )
                        errores_emp += 1
                    else:
                        ...
                        if row["UNITATS REALS"] != row["UNITATS DISPONIBLES"]:
                            warn(
                                f"Producto {row['PRODUCTE']}: REALS({row['UNITATS REALS']}) ≠ DISPONIBLES({row['UNITATS DISPONIBLES']}) con pendientes=0"
                            )
            ...
            if tiene_stock_precalc:
                info(
                    "Usando columna A_STOCK_HES precalculada para verificación de stock"
                )
            ...
            for producto in sorted(productos):
                ...
                if tiene_stock_precalc:
                    ...
                    stocks = pd.to_numeric(ops["A_STOCK_HES"], errors="coerce").fillna(
                        0
                    )
                ...
                if min_stock < 0:
                    err(
                        f"Producto {c(producto, Color.YELLOW)}: STOCK NEGATIVO → mínimo: {c(str(min_stock), Color.RED)} (en {min_fecha}, ref: {min_ref})"
                    )
                    self._registrar(
                        empresa,
                        "STOCK_NEGATIVO",
                        f"{producto}: stock mínimo {min_stock} en {min_fecha}",
                    )
                    errores_emp += 1
                    stock_negativo_total += 1
                else:
                    ok(
                        f"Producto {producto}: stock mínimo OK ({min_stock}), stock final: {stock_final}"
                    )
                ...
                if not inv_prod.empty:
                    ...
                    if stock_final != unidades_reales:
                        err(
                            f"Producto {producto}: stock historial ({stock_final}) ≠ UNITATS REALS inventario ({unidades_reales})"
                        )
                        self._registrar(
                            empresa,
                            "STOCK_DISCREPANCIA",
                            f"{producto}: historial {stock_final} vs inventario {unidades_reales}",
                        )
                        errores_emp += 1
            ...
            if errores_emp == 0:
                ok(
                    c(
                        f"EMPRESA {empresa}: SIN ERRORES EN INVENTARIO",
                        Color.GREEN + Color.BOLD,
                    )
                )
            else:
                err(
                    c(
                        f"EMPRESA {empresa}: {errores_emp} ERROR(ES) EN INVENTARIO",
                        Color.RED + Color.BOLD,
                    )
                )
    ...
    def informe_final(self, exportar_csv=True):
        ...
        print(
            f"  {c('✓', Color.GREEN)} Verificaciones correctas : {c(str(self.correctos_total), Color.GREEN)}"
        )
        print(
            f"  {c('⚠', Color.YELLOW)} Avisos                  : {c(str(self.avisos_total), Color.YELLOW)}"
        )
        print(
            f"  {c('✗', Color.RED)} Errores totales          : {c(str(self.errores_total), Color.RED)}"
        )
        ...
        for exp, datos in sorted(self.resumen_por_alumno.items()):
            ...
            color_nota = (
                Color.GREEN if nota >= 7 else Color.YELLOW if nota >= 5 else Color.RED
            )
            ...
            print(
                f"  {exp:<12} {empresa:<35} {n_err:>5} {n_avi:>5} {n_ok:>5}  {nota_str}"
            )
            filas_csv.append(
                {
                    "EXPEDIENTE": exp,
    ...
    for nombre in archivos:
        ...
        if df is not None:
            ...
            for i, col in enumerate(df.columns):
                print(
                    f"    {c(str(i).rjust(2), Color.GRAY)}  {c(repr(col), Color.CYAN)}"
                )
            if len(df) > 0:
...
""",
    ...
    directorio = input(
        c("  Directorio con los CSVs [Enter = actual]: ", Color.CYAN)
    ).strip()
    if not directorio:
        directorio = "."
    ...
    fecha_entrega = input(
        c(
            "  Fecha de entrega del trabajo (dd/mm/aaaa) [Enter = sin fecha]: ",
            Color.CYAN,
        )
    ).strip()
    ...
    if opcion == "1":
        corrector.corregir_compras(fecha_entrega)
    elif opcion == "2":
        corrector.corregir_ventas(fecha_entrega)
    elif opcion == "3":
        corrector.corregir_inventario(fecha_entrega)
    elif opcion == "4":
        corrector.corregir_compras(fecha_entrega)
        corrector.corregir_ventas(fecha_entrega)
        corrector.corregir_inventario(fecha_entrega)
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if modulo in ("compras", "c", "1"):
            corrector.corregir_compras(fecha)
        elif modulo in ("ventas", "v", "2"):
            corrector.corregir_ventas(fecha)
        elif modulo in ("inventario", "inv", "3"):
            corrector.corregir_inventario(fecha)
            ...
            print(
                "Uso: python corrector.py [compras|ventas|inventario|todo|diagnostico] [directorio] [fecha]"
            )
            return
        corrector.informe_final()
    else:
