...
class CorrectorEmpresaula:
    ...
    def corregir_inventario(self, fecha_entrega_str=None):
        ...
        df_inv['UNITATS DISPONIBLES']   = pd.to_numeric(df_inv['UNITATS DISPONIBLES'], errors='coerce').fillna(0).astype(int)
        df_inv['UNITATS PENDENTS ENTRAR'] = pd.to_numeric(df_inv['UNITATS PENDENTS ENTRAR'], errors='coerce').fillna(0).astype(int)
        df_inv['UNITATS PENDENTS SORTIR'] = pd.to_numeric(df_inv['UNITATS PENDENTS SORTIR'], errors='coerce').fillna(0).astype(int)
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        # python corrector.py <modulo> <directorio> [fecha_entrega]
        modulo    = sys.argv[1].lower()  # compras | ventas | inventario | todo
        ...
        if modulo in ('compras', 'c', '1'):
            ...
            print("Uso: python corrector.py [compras|ventas|inventario|todo] [directorio] [fecha_entrega]")
        corrector.informe_final()
    else:
        menu_principal()
