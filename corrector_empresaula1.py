...
def normalizar_fecha(valor, formatos=None):
    ...
    for fmt in formatos:
        try:
            return datetime.strptime(s[:len(fmt.replace('%Y','2000').replace('%m','01').replace('%d','01').replace('%H','00').replace('%M','00').replace('%S','00'))], fmt).date()
        except:
            pass
    # Intentar pandas
    try:
...
def leer_csv(ruta, separador=','):
    ...
    try:
        df = pd.read_csv(ruta, sep=separador, encoding='utf-8', dtype=str,
        ...
        try:
            df = pd.read_csv(ruta, sep=separador, encoding='latin-1', dtype=str,
                             quotechar='"', skipinitialspace=True)
            df.columns = df.columns.str.strip().str.replace('"', '')
            return df
...
class CorrectorEmpresaula:
    ...
    def corregir_ventas(self, fecha_entrega_str=None):
        ...
        for empresa in sorted(empresas):
            ...
            self._verificar_facturas_venta(empresa, peds_emp, facs_emp, reales_emp, errores_emp)

        subheader("Verificación de ventas completada")


    def _verificar_facturas_venta(self, empresa, peds_emp, facs_emp, reales_emp, errores_emp):
    ...
    def corregir_inventario(self, fecha_entrega_str=None):
        ...
        MAP_INV = {
            ...
            'PRODUCTE':                 ['PRODUCTE', 'PRODUCTO', 'Producto', 'producto', 'Product', 'PRODUCT'],
            'COST UNITARI':             ['COST UNITARI', 'COSTE UNITARIO', 'Cost unitari', 'Coste unitario',
                                         'COST_UNITARI', 'Unit Cost', 'Cost', 'COST'],
            'IMPORT':                   ['IMPORT', 'IMPORTE', 'Import', 'Importe', 'VALUE', 'Value', 'VALOR'],
            'UNITATS REALS':            ['UNITATS REALS', 'UNIDADES REALES', 'Unitats reals', 'Unidades reales',
                                         'On Hand', 'On hand', 'QTY', 'Qty', 'QUANTITY'],
            'UNITATS DISPONIBLES':      ['UNITATS DISPONIBLES', 'UNIDADES DISPONIBLES', 'Forecasted Quantity',
                                         'Forecasted', 'Disponible', 'Available'],
            'UNITATS PENDENTS ENTRAR':  ['UNITATS PENDENTS ENTRAR', 'UNIDADES PENDIENTES ENTRAR',
                                         'Incoming', 'incoming', 'INCOMING'],
            'UNITATS PENDENTS SORTIR':  ['UNITATS PENDENTS SORTIR', 'UNIDADES PENDIENTES SALIR',
                                         'Outgoing', 'outgoing', 'OUTGOING'],
        }
        # Historial
        MAP_HIS = {
            'EMPRESA':   ['EMPRESA', 'Empresa', 'empresa', 'COMPANY', 'Company'],
            'Producto':  ['Producto', 'PRODUCTO', 'PRODUCTE', 'Product', 'PRODUCT', 'producte'],
            'Fecha':     ['Fecha', 'FECHA', 'Date', 'DATE', 'Scheduled Date', 'date'],
            'Referencia':['Referencia', 'REFERENCIA', 'Reference', 'REFERENCE', 'Ref', 'REF'],
            'Desde':     ['Desde', 'DESDE', 'From', 'FROM', 'Source Location', 'Source'],
            'A':         ['A', 'a', 'To', 'TO', 'Destination Location', 'Destination', 'Dest'],
            'Cantidad':  ['Cantidad', 'CANTIDAD', 'Quantity', 'QUANTITY', 'QTY', 'Qty', 'Done'],
        }
        ...
        for empresa in sorted(empresas):
            ...
            for producto in sorted(productos):
                ...
                for _, op in ops.iterrows():
                    # Ignorar operaciones de ajuste sin cantidad real
                    if op['Cantidad'] == 0:
                        continue
                    ...
                    if es_entrada:
                        ...
                        stock -= op['Cantidad']
                    # Si no es ninguno (ajuste interno), ignorar
...
""", Color.BOLD + Color.BLUE))
    ...
    fecha_entrega = input(c("  Fecha de entrega del trabajo (dd/mm/aaaa) [Enter = sin fecha]: ", Color.CYAN)).strip()
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if modulo in ('compras', 'c', '1'):
            ...
            print("Uso: python corrector.py [compras|ventas|inventario|todo|diagnostico] [directorio] [fecha]")
            return
        corrector.informe_final()
    else:
        menu_principal()
